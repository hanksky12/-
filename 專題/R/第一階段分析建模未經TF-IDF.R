...
avg_data_ID=read_csv("C:/Users/sky/iii_ETL/avg_final_2v_v250_w10_sg1_min4_a0.02_hs0.csv")
...
#取出62=>8成5 取23=>7成  取8=>5成
compose=predict(pca_avg)
compose62=compose[,c(1:62)]
compose23=compose[,c(1:23)]
compose8=compose[,c(1:8)]
#3-252 原始向量 253-314 compose向量
avg_complete62=cbind(avg_complete,compose62)
avg_complete23=cbind(avg_complete,compose23)
avg_complete8=cbind(avg_complete,compose8)
#62變數要帶入的DATA
...
#可選模型 指標 資料集
library(factoextra)
fviz_nbclust(avg_complete8[,-seq(from=1,to=252)], 
             FUNcluster = kmeans,  # hierarchical clustering
             method = "wss",     # "silhouette" "wss" "gap_stat" total within sum of square
             k.max = 6        # max number of clusters to consider
             )
library(pamk)
library(fpc)
library(cluster)
#PAM方法  kmeans改良 抓實際點當中心
pamk.best <- pamk(avg_complete23[,-seq(from=1,to=252)])
pamk.best$nc

#密度式分群
ds = dbscan(data = dist(avg_complete23[,-seq(from=1,to=252)]),eps= 0.2, MinPts = 3, method="dist")

plot(ds, avg_complete23[,-seq(from=1,to=252)])

clusplot(pam(avg_complete23[,-seq(from=1,to=252)], pamk.best$nc))
#kmeans 出圖
km.res <- kmeans(avg_complete23[,-seq(from=1,to=252)],3)
fviz_cluster(km.res, data = avg_complete23[,-seq(from=1,to=252)])

#檢測指標
gap_stat <- clusGap(x = avg_complete[,-seq(from=1,to=252)],FUNcluster = hcut, nstart = 25, K.max = 8, B = 50)
