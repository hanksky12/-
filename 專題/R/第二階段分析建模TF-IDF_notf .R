#================================讀檔======================
library(readr)
setwd("F:/資策會/專題/個人資料/BT算法")
avg_data_ID=read_csv("C:/Users/sky/iii_ETL/fasttext_cut1230_cut1230_v17_w5_sg0_min3_i10_no_around_power1_2_AVG.csv")
...
compose8=compose[,c(1:8)]
# 
# #3-252 原始向量 253-314 compose向量
# avg_complete=cbind(avg_complete,compose)
# avg_complete10=cbind(avg_complete,compose10)
# 
#62變數要帶入的DATA
avg_complete[,-seq(from=1,to=252)]
# #23變數要帶入的DATA
# #avg_complete23[,-seq(from=1,to=252)]
# #8變數要帶入的DATA
#avg_complete8[,-seq(from=1,to=252)]
# #============================階層式分群(bottom-up)=====================
# #62變數+歐式只有沃德和complete能用==>一張一張看   笨蛋!!!!!
# #hc=hclust(dist(avg_complete[,-seq(from=1,to=252)], method="euclidean"), method="ward.D2")
# #hc=hclust(dist(avg_complete[,-seq(from=1,to=252)], method="euclidean"), method="single")
# #hc=hclust(dist(avg_complete[,-seq(from=1,to=252)], method="euclidean"), method="complete")
# #hc=hclust(dist(avg_complete[,-seq(from=1,to=252)], method="euclidean"), method="average")
# #hc=hclust(dist(avg_complete[,-seq(from=1,to=252)], method="euclidean"), method="centroid")
# #hc=hclust(dist(avg_complete[,-seq(from=1,to=252)], method="manhattan"), method="ward.D2")
# #hc=hclust(dist(avg_complete[,-seq(from=1,to=252)], method="manhattan"), method="complete")
...
#  return(BT_k_3_see)
# }
# BT=BT_k_3(compose62=compose[,c(1:62)],3)
# # #@@@@@@@@@@@@@@@@BT_k=3畫圖用的DF
...
# }
# 
# BT_group_2_6(avg_complete62[,-seq(from=1,to=252)],62)
# BT_group_2_6(avg_complete23[,-seq(from=1,to=252)],23)
# BT_group_2_6(avg_complete8[,-seq(from=1,to=252)],8)
# 
...
pamk.best <- pamk(avg_complete[,-seq(from=1,to=2)])
pamk.best$nc

#密度式分群
ds = dbscan(data = dist(avg_complete[,-seq(from=1,to=2)]),eps= 1.7, MinPts = 10, method="dist")

clusplot(ds, avg_complete[,-seq(from=1,to=2)])

clusplot(pam(avg_complete[,-seq(from=1,to=2)], pamk.best$nc))

#檢測指標
gap_stat <- clusGap(x = avg_complete[,-seq(from=1,to=2)],FUNcluster = kmeans, nstart = 25, K.max = 12, B = 5)
fviz_gap_stat(gap_stat)

#kmeans 出圖
km.res <- kmeans(avg_complete[,-seq(from=1,to=2)],3)
fviz_cluster(km.res, data = avg_complete[,-seq(from=1,to=2)])


library(kernlab)

sc <- specc(avg_complete[,-seq(from=1,to=2)], centers=2)
plot(avg_complete[,-seq(from=1,to=2)], col=sc, pch=4)            # estimated classes (x)
points(avg_complete[,-seq(from=1,to=2)], col=obj$classes, pch=5)
