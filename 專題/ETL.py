...
def csv_to_out(ingredient_names_clean):
    with open('free_food_same_name.csv', 'a+', newline='', encoding="utf-8") as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow([ingredient_names_clean])
            ...
            with open(path + "/" + i, "r", encoding="utf-8") as f:
                try:
                    ...
                    for i in list_ingredients:
                        ...
                        # 輸出給裕璋 建立食材同義詞庫
                        #csv_to_out(ingredient_names_clean)

                        #清理單位
                        ingredient_units=i["ingredient_units"]
                        ingredient_units_clean=clean_unit(ingredient_units)
                        print(ingredient_units_clean,"        ",ingredient_names,"       ",ingredient_units_clean)
